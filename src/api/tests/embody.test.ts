...
describe('embody', () => {
  describe('async behavior', () => {
    it('.steps returns a Promise', () => {
      const result = embody.tracer(txtCharsTracer).code('ab').steps;

      expect(result).toBeInstanceOf(Promise);
    });

    it('.steps resolves to steps array', async () => {
      const steps = await embody.tracer(txtCharsTracer).code('ab').steps;
    ...
    it('.resolvedConfig returns sync (not Promise)', () => {
      const result = embody.tracer(txtCharsTracer).resolvedConfig;
  ...
  describe('basic chaining', () => {
    ...
    it('.code().tracer().steps works (order independent)', async () => {
      const steps = await embody.code('ab').tracer(txtCharsTracer).steps;

      expect(steps).toHaveLength(2);
    });

    it('.steps is a lazy getter (traces on access)', async () => {
      const chain = embody.tracer(txtCharsTracer).code('ab');
  ...
  describe('type validation (eager, sync)', () => {
    it('.tracer() throws TracerInvalidError for non-object', () => {
      expect(() => embody.tracer(123 as unknown as TracerModule)).toThrow(TracerInvalidError);
    });

    it('.tracer() message says "plain object" for non-object input', () => {
      expect(() => embody.tracer(null as unknown as TracerModule)).toThrow(/plain object/);
    });

    it('.code() throws immediately on non-string', () => {
      expect(() => embody.code(456 as unknown as string)).toThrow(/expected a string/);
    });

    it('.code() throws with descriptive message including actual type', () => {
      expect(() => embody.code(undefined as unknown as string)).toThrow(/got undefined/);
    });
  });

  describe('validation (sync throws from getters)', () => {
    it('.tracer() throws TracerInvalidError for invalid TracerModule', () => {
      expect(() => embody.tracer({} as unknown as TracerModule)).toThrow(TracerInvalidError);
    });

    it('.steps throws when tracer missing', () => {
      expect(() => embody.code('ab').steps).toThrow(/tracer.*required/i);
    });

    it('.steps throws when code missing', () => {
      expect(() => embody.tracer(txtCharsTracer).steps).toThrow(/code.*required/i);
    });
    ...
    it('.steps rejects with StepsInvalidError for invalid tracer output (async)', async () => {
      ...
      await expect(embody.tracer(badTracer).code('hello').steps).rejects.toBeInstanceOf(
        StepsInvalidError,
      );
    });

    it('.steps rejects with ParseError from tracer module (async)', async () => {
      await expect(embody.tracer(txtCharsTracer).code('‽').steps).rejects.toBeInstanceOf(
        ParseError,
      );
    });
  });
  ...
  describe('immutability', () => {
    it('config is deep cloned on entry — caller mutation does not affect chain', async () => {
      ...
      const chain = embody.tracer(txtCharsTracer).code('ab').config(config);
  ...
  describe('resolvedConfig getter (sync)', () => {
    it('.resolvedConfig returns options with tracer defaults', () => {
      const chain = embody.tracer(txtCharsTracer);
    ...
    it('.resolvedConfig includes user-provided options merged with defaults', () => {
      const chain = embody.tracer(txtCharsTracer).config({ options: { remove: ['a'] } });
      const resolved = chain.resolvedConfig;
    ...
    it('.resolvedConfig does not require code', () => {
      const resolved = embody.tracer(txtCharsTracer).resolvedConfig;
